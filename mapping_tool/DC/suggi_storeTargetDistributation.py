            ...
            if key in ["TM","Store","Category"]:
                ...
                for i,sale in enumerate(sale_data):
                    for j,product in enumerate(sale["product"]):
                        if product["purchaseProductDetails"]["category"]["name"] in sub_category_gross.keys():
                            sub_category_gross[product["purchaseProductDetails"]["category"]["name"]]+= product["sellingprice"]*product["soldqty"]
                        else:
                             sub_category_gross[product["purchaseProductDetails"]["category"]["name"]]= product["sellingprice"]*product["soldqty"]
                ...
                for sale in sale_data:
                    ...
                    for product in sale["product"]:
                        if "category" in product["purchaseProductDetails"].keys() and product["purchaseProductDetails"]["category"]["name"] == row["Category"]:
                            storeTargetPerProduct = (product["sellingprice"]*product["soldqty"])*float((row[key]*100000))/sub_category_gross[product["purchaseProductDetails"]["category"]["name"]]
